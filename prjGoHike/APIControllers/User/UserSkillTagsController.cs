...
public sealed class UserSkillTagsController : UserApiControllerBase
{
    ...
    public async Task<ActionResult> SetDisplayed(long tagId, SetSkillTagDisplayRequest request, CancellationToken ct)
    {
        ...
        // Hold the user's update lock until commit so concurrent display requests
        // count the skills saved by the previous request, even across API instances.
        await _context.Database.ExecuteSqlInterpolatedAsync(
            $"SELECT user_id FROM dbo.users WITH (UPDLOCK, HOLDLOCK) WHERE user_id = {userId}", ct);
